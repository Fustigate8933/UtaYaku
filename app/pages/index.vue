<template>
  <div
    class="flex items-center flex-col justify-center h-full w-full gap-5 relative"
  >
    <div
      class="max-w-5xl flex items-center flex-col justify-center h-full w-full gap-10 py-[4rem]"
    >
      ...
      <div v-if="useAi" class="">
        <div
          class="w-full flex text-center justify-center items-center gap-3 mb-2"
        >
          <h1 class="">HuggingChat Credentials:</h1>
          <div class="flex flex-col gap-1">
            <input
              v-model="huggingchatEmail"
        ...
        <ul class="ml-3 text-sm text-gray-400">
          <li>
            * Please provide your credentials from
            <span class="text-blue-300 underline"
              ><a href="https://huggingface.co/chat/" target="_blank"
                >HuggingChat</a
              ></span
            >(free) as ai analysis requires it.
          </li>
          <li>
            * It's best to use a throwaway account for concerns of privacy and
            chat modification.
          </li>
        </ul>
      </div>
      <div class="w-full flex gap-10 text-center justify-center text-lg">
        ...
        <div class="flex flex-col gap-1">
          <label
            >アーティスト
            <span class="text-sm text-gray-400">(Artist)</span></label
          >
          <input
            v-model="artist_name"
            class="rounded-lg px-2 py-1 text-black"
        ...
        <div class="flex flex-col gap-1">
          ...
          <label class="w-[100px]"
            ><span class="text-sm text-gray-400">(Search)</span></label
          >
          <button
            class="h-full rounded-lg hover:cursor-pointer border-2 border-gray-400 hover:border-white active:border-gray-400"
            @click="getMetaData"
          >
            検索
          </button>
